...
static LAYOUTS: &[EmbeddedLayout] = &[
    EmbeddedLayout { id: "k03", name: "K:03", json: K03_JSON },
    EmbeddedLayout { id: "imperial44", name: "Imperial44", json: IMPERIAL44_JSON },
    EmbeddedLayout { id: "op36", name: "Omega Point 36", json: OP36_JSON },
];
...
pub fn lookup_layout(device_name: &str) -> Option<(&'static EmbeddedLayout, Vec<PhysicalKey>)> {
    ...
    let layout = LAYOUTS.iter().find(|l| {
        ...
        dn.contains(&id) || dn.contains(&name) || dn_alnum.contains(&id_alnum) || dn_alnum.contains(&name_alnum)
    })?;
...
pub fn build_layout_from_zmk(
    ...
    let (keys, rows, cols) = if let Some(pl) = phys_layout {
        let mut max_row = 0u8;
        let mut max_col = 0u8;
        let mut keys: Vec<PhysicalKey> = pl.keys.iter().enumerate().map(|(i, k)| {
            // ZMK uses centiunits (1/100 KLE unit)
            let x = k.x as f32 / 100.0;
            let y = k.y as f32 / 100.0;
            let w = if k.width > 0 { k.width as f32 / 100.0 } else { 1.0 };
            let h = if k.height > 0 { k.height as f32 / 100.0 } else { 1.0 };
            ...
            PhysicalKey { x: final_x, y: final_y, w, h, row, col, label: format!("{i}"), rotation: 0.0, rotation_x: 0.0, rotation_y: 0.0 }
        }).collect::<Vec<_>>();
        ...
        // ZMK dtsi has inherent asymmetry in thumb Y coordinates
        {
            ...
            let left_indices: Vec<usize> = pl.keys.iter().enumerate()
                .filter(|(_, k)| k.r > 0).map(|(i, _)| i).collect();
            let right_indices: Vec<usize> = pl.keys.iter().enumerate()
                .filter(|(_, k)| k.r < 0).map(|(i, _)| i).collect();
            if !left_indices.is_empty() && !right_indices.is_empty() {
                let left_avg_y = left_indices.iter().map(|&i| keys[i].y).sum::<f32>() / left_indices.len() as f32;
                let right_avg_y = right_indices.iter().map(|&i| keys[i].y).sum::<f32>() / right_indices.len() as f32;
                let mid_y = (left_avg_y + right_avg_y) / 2.0;
                for &i in &left_indices { keys[i].y += mid_y - left_avg_y; }
                for &i in &right_indices { keys[i].y += mid_y - right_avg_y; }
            }
        }
        ...
        let keys: Vec<PhysicalKey> = (0..key_count).map(|i| PhysicalKey {
            x: (i % cols) as f32,
            y: (i / cols) as f32,
            w: 1.0, h: 1.0,
            row: (i / cols) as u8,
    ...
    let name = phys_layout.map(|p| p.name.clone()).unwrap_or_else(|| "ZMK Keyboard".to_string());
