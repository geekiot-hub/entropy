...
pub fn lookup_layout(device_name: &str) -> Option<(&'static EmbeddedLayout, Vec<PhysicalKey>)> {
    ...
    let layout = LAYOUTS.iter().find(|l| {
        dn.contains(&l.id.to_lowercase()) || dn.contains(&l.name.to_lowercase())
    })?;
...
pub fn build_layout_from_zmk(
    ...
    let (keys, rows, cols) = if let Some(pl) = phys_layout {
        let mut max_row = 0u8;
        let mut max_col = 0u8;
        let keys: Vec<PhysicalKey> = pl.keys.iter().enumerate().map(|(i, k)| {
            ...
            PhysicalKey { x, y, w, h, row, col, label: format!("{i}"), rotation: r, rotation_x: rx, rotation_y: ry }
        }).collect();
