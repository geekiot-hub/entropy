...
pub fn zmk_binding_tooltip(binding: &ZmkBinding, behaviors: &[BehaviorInfo], layer_names: &[String]) -> String {
    ...
    match name {
        ...
        "Studio Unlock"    => "Studio Unlock — allow ZMK Studio to edit the keymap".to_string(),
        "External Power"   => match p1 { 0 => "External Power OFF".to_string(), _ => "External Power ON".to_string() },
        "Bluetooth" => match p1 {
            0 => "BT Clear — forget current Bluetooth profile".to_string(),
            1 => "BT Clear All — forget all Bluetooth profiles".to_string(),
            2 => "BT Next — switch to next Bluetooth profile".to_string(),
            3 => "BT Previous — switch to previous Bluetooth profile".to_string(),
            4 => format!("BT Select profile {}", p2),
            _ => format!("Bluetooth action {}", p1),
        },
        "Output Selection" => match p1 {
            0 => "Output: USB — send keystrokes via USB".to_string(),
